...
# 본문의 이 부분에 이어지는 코드: 사실은 이런 관찰단위로 행을 만드는 것이 더 자연스러운 것이다. 왜냐하면 데이터가 모아지는 대로 데이터를 정리하는 것이기 때문이다.
...
## 두 개의 데이터 프레임을 하나로 합친 경우: 불가피한 중복을 보여주는 jdf
jdf <- join(ddf, demo, by="names")
jdf <- jdf[, c("names", "weight", "height", "followup", "bp", "pulse")]
jdf


## 정리되지 않은 messy 형태의 newdf, 정리된 ddf
## 각각을 messy, tidy라는 객체로 이하 설명됨.
...
## tidy 데이터 프레임 tidy로 ggplot 그래프를 그려봄 
library(ggplot2)
